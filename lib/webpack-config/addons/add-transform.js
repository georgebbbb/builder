...
const makeExtensionPattern = extension => new RegExp(
  `\\.${regexpEscape(extension)}\$`)
      ...
      $set: options.presets.map(
        preset => {
          ...
          if (preset && preset[0] === 'es2015') {
            return ['es2015', utils.extend({}, preset[1], {
              'modules': false
            })]
          }
          return preset
  ...
  const rule = {
    ...
    use: loaderList.map(
      ({
        ...
        options = loader === 'babel-loader' ? adaptBabelLoaderOptions(
          options) : options
        return {
          loader, options
        }
...
const makeReactBabelOptions = config => utils.extend({
    presets: ['es2015', 'react'].map(name => require.resolve('babel-preset-' +
      name)),
...
module.exports = (config, key, transform) => {
  ...
  if (!transform || !transform.transformer || typeof transform.transformer !==
    'string') {
    throw new TypeError(
      `Invalid transform info: ${JSON.stringify(transform)}`)
  }
  ...
  switch (transform.transformer) {
    ...
    case transforms.tsx:
      const babelOptions = (
        transform.transformer === transforms.tsx ? makeReactBabelOptions(
